...
description: AI 기반 법률 뉴스 분석 시스템 개발을 위한 단계별 프로세스 가이드. 뉴스 크롤링, LLM 기반 소송금융 적합도 분석, 웹 대시보드, 엑셀 내보내기를 포함한 PoC/MVP 개발. Django + Celery + Next.js 풀스택 개발 워크플로우 제공.
---

# Law News Analyzer - 개발 프로세스 가이드

AI 기반 법률 분쟁 사건 자동 발굴 시스템의 PoC/MVP 개발을 단계별로 진행하기 위한 종합 가이드입니다.
각 단계를 완료한 후 사용자의 테스트와 승인을 받아 다음 단계로 진행합니다.
...
- **프론트엔드**: React(vite) 최신버전 + Tailwind CSS + shadcn/ui
- **데이터베이스**: 개발시: sqLite,  이후 배포시 -> PostgreSQL
- **비동기 작업**: Celery + Redis
- **크롤링**: requests + BeautifulSoup4 (또는 네이버 뉴스 API)
- **LLM**: OpenAI GPT-4o 또는 Gemini API
- **엑셀**: openpyxl
